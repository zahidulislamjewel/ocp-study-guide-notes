...
**_Variables hold the state of the program_**

**_Methods operate on that state_**
...
There may be a minor impact on compilation time, as the compiler has to search through all the classes in a package to find the ones being used, rather than going directly to a specified class file. However, with modern compilers and IDEs, this difference is generally trivial and unnoticeable for most projects. 
...
When you’re counting instance initializers, keep in mind that they cannot exist inside a method.

- Fields and instance initializer blocks are run in the order in which they appear in the file.
- The constructor runs after all fields and instance initializer blocks have run.

**Data Types**
Eight built in data types (primitive types)
...
Remember, the compiler is concerned only if you try to use uninitialized local variables; it doesn’t mind the ones you never use.
...
On the other hand, a class variable is one that is defined on the class level and shared among all instances of the class. It can even be publicly accessible to classes outside the class and doesn’t require an instance to use.
...
We can use this feature only for local variables. 
In Java, var is still a specific type defined at compile time. It does not change type at runtime.

The initial value used to determine the type of var needs to be part of the same statement. That means, we need to initialize the variable declared with var keyword in the same line where it is declared. Otherwise, the compiler does not know what to make of them i.e. fails to infer the variable type. 

Furthermore, ` var n = null;` is not allowed too. The designers of Java decided it would be better not to allow var for `null` than to have to guess at intent. Var also cannot be used with constructor parameters, method parameters, or instance variables. Using var in one of these places is a good exam trick to see if you are paying attention. Remember that var is used only for local variable type inference! So, the following method signature is not legal,
...
- There’s one last rule you should be aware of: `var` is not a reserved word and allowed to be used as an identifier. 
